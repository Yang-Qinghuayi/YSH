...
onMounted(async () => {
  const { playlist: radarPlaylist } = await getPlaylistDetail(
    specialType.radar.id
  );
...
<template>
  <v-dialog v-model="show">
    ...
    >
        ...
        >
          ...
          <div class="d-flex flex-column text-start ml-3">
            <div class="d-flex gap-1 w-full">
              <span class="font-weight-medium line-clamp-1"
                >{{ profile.nickname }}
              </span>
            </div>
          </div>
      ...
      <v-list-item
        height="64"
        class="mx-6 px-3 bg-surfaceVariant rounded-lg"
        @click="dispatch('podcast')"
      >
        <template #prepend>
      ...
      <v-list-item
        height="64"
        class="mx-6 px-3 mt-2 bg-surfaceVariant rounded-lg"
