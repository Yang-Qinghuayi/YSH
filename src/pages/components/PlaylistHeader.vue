  ...
  <div class="d-flex flex-column gap-6">
    <div class="drag-area d-flex mx-n4" :class="smAndUp ? '' : 'flex-column'">
      ...
      <div
        ...
        :style="{
          background:
            'linear-gradient(360deg, rgba(var(--v-theme-surface), 1) 0%,rgba(0,0,0,0) 100%)',
        }"
      >
        <back-btn
          class="align-self-start mb-auto mx-4 mt-4"
          variant="tonal"
          color="primary"
        />
        <div class="no-drag-area">
          <div class="d-flex flex-column gap-2 mx-6 mb-2">
            ...
            <div class="d-flex py-2" :class="{ 'justify-center': !smAndUp }">
              <div
                class="d-flex flex-column align-center pr-4"
                :style="{ minWidth: '96px' }"
              >
              ...
              <div
                class="d-flex flex-column align-center pl-4"
                :style="{ minWidth: '96px' }"
              >
                <span class="text-body-1 font-weight-medium">{{
                ...
                <v-dialog v-model="showDeleteAlert" persistent>
                  ...
                  >
                    ...
                    <v-card-actions>
                      ...
                      <v-btn color="primary" variant="plain" @click="del">
                        {{ t("common.agree") }}
                      </v-btn>
                    </v-card-actions>
                  </v-card>
    ...
    <v-dialog v-model="showEdit" :scrollable="true">
      ...
      >
        ...
        <v-card-actions class="justify-end">
          <v-btn color="primary" variant="plain" plain @click="cancel">
            {{ t("common.cancel") }}
          </v-btn>
          <v-btn color="primary" variant="plain" plain @click="edit">
            {{ t("common.confirm") }}
          </v-btn>
        </v-card-actions>
      </v-card>
...
import { formatDuring, formatNumber } from "@/utils/fn";
...
async function play() {
  if (props.playlist) {
    ...
    playQueueStore.updatePlayQueue(
      props.playlist.id,
      "playlist",
      props.playlist.name,
...
async function edit() {
  const { code } = await updatePlaylist(
    props.playlist.id,
    editForm.title,
