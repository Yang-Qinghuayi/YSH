...
export class Player {
  ...
  private init() {
    ...
    this.initStoreEvent();
    if (this.track?.id) {
      this.updatePlayerTrack(
        this.track.id,
        false,
        false,
        false,
        this.track.source?.from
      );
    }
  }
  ...
  async updatePlayerTrack(
    ...
    try {
      const { track, trackMeta, lyric } = await getTrackDetail(
        trackId,
        from?.type === "program"
      );
      // restore common mode
      ...
      if (trackMeta.url) {
        ...
        this.howler?.unload();
        this.howler = null;
        const url = trackMeta.sourceFromUnlockMusic
          ? trackMeta.url
          : toHttps(trackMeta.url);
  ...
  private initSound(src: string) {
    ...
    const sound = new Howl({
      ...
      src,
      html5:
        this.track?.meta?.sourceFromUnlockMusic ||
        !this.settingStore.visualization, // web audio 用 xhr 方式拉取音频，解锁音乐可能会存在跨域，强制 html5 加载
      preload: "metadata",
      format: ["mp3", "flac"],
      onplay: () => {
        this.pauseProgress();
